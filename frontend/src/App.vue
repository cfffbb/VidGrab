<template>
  <div class="min-h-screen flex flex-col bg-bg-main">
    ...
    <!-- Tab 导航栏 -->
    <div class="sticky top-16 z-40 bg-bg-card/80 backdrop-blur-md border-b border-border">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-1">
          <button
            ...
            :class="[
              'px-4 sm:px-6 py-3 text-sm font-medium transition-colors border-b-2 -mb-px',
              activeTab === tab.id
    ...
    <main class="flex-1">
      ...
      <template v-if="activeTab === 'single'">
        ...
        />
        <!-- 视频信息 + AI 总结：左右双栏同屏布局 -->
        <section v-if="videoData" class="py-4 sm:py-6">
      ...
      <template v-else-if="activeTab === 'batch'">
        <BatchDownload :user="currentUser" @need-login="showAuthModal('login')" @need-vip="handleOpenVip" />
      </template>

      <!-- 音频提取模式 -->
      <template v-else-if="activeTab === 'audio'">
        <AudioExtractor :user="currentUser" @need-login="showAuthModal('login')" @need-vip="handleOpenVip" />
      </template>
    </main>
    <AppFooter />
      ...
      <div v-if="paymentToast" class="fixed top-20 left-1/2 -translate-x-1/2 z-[200] animate-toast-in">
        <div :class="[
          ...
          paymentToast === 'success'
            ? 'bg-bg-card border-success/50 text-success'
            : 'bg-bg-card border-warning/50 text-warning'
        ]">
          <svg v-if="paymentToast === 'success'" class="w-5 h-5 text-success" fill="none" stroke="currentColor" viewBox="0 0 24 24">
