<template>
  <div class="glass-card rounded-2xl overflow-hidden h-full">
    ...
    <div class="flex flex-col gap-5 p-5 sm:p-6">
      <!-- 缩略图 -->
      <div class="relative w-full aspect-video rounded-xl overflow-hidden bg-bg-hover">
        <img
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="flex flex-wrap items-center gap-3 text-sm text-text-secondary mb-3">
          ...
          <!-- 平台标签 -->
          <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-primary/20 text-primary rounded-full text-xs font-medium">
            {{ video.platform }}
          </span>
          <!-- 播放量 -->
    ...
    <div class="border-t border-border px-5 sm:px-6 py-5">
      ...
      <div class="grid grid-cols-1 gap-2.5">
        ...
        >
          <div class="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center"
            :class="selectedFormat === fmt.format_id ? 'bg-primary text-white' : 'bg-bg-hover text-text-muted'">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <div class="mt-6 flex flex-col items-stretch gap-3">
        ...
        <button
          ...
          class="w-full inline-flex items-center justify-center gap-2 h-12 px-10 rounded-full gradient-bg text-white font-medium text-base transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg cursor-pointer"
        >
        ...
        <button
          ...
          class="w-full inline-flex items-center justify-center gap-2 h-12 px-8 rounded-full border border-accent text-accent hover:bg-accent hover:text-white font-medium text-base transition-all shadow-sm hover:shadow-md cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
        >
